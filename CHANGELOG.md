...
- **Mobile readability + tactile palette v2:** strengthened phone-only small-label readability with heavier Manrope, tighter tracking, and darker ink; redesigned dresscode swatches as tactile glass/porcelain controls with convex color buttons, internal highlights, and JS-managed active gold ring state.
- **Dresscode desktop compact + quieter palette:** reduced desktop dresscode shell scale and visual weight so the palette remains visible, then replaced the app-like pill rail with separate floating jewelry-style swatches and a thin active gold ring.
- **Hero mobile date + preload cleanup:** enlarged and strengthened mobile hero date/time for readability and removed the unused initial `/models/dress.glb` preload because the dress model is lazy-loaded on dresscode entry.
