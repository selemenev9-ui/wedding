...
- **Open Graph / share**: `index.html` head now uses absolute HTTPS metadata for stable crawlers/cards: `og:url` = **`https://katyartemwedding.ru/`**, `og:site_name`, `og:image` + `og:image:secure_url` = **`https://katyartemwedding.ru/og.webp?v=2`**, `og:image:type` `image/webp`, dimensions **1200×630**, `og:type: website`. Added `twitter:card=summary_large_image` + `twitter:title/description/image`. Asset remains **`public/og.webp`**.
