...
- **Open Graph / share**: `index.html` head — `og:title`, `og:description`, `og:image` → **`/og.webp`**, dimensions **1200×630**, `og:type: website`. Asset: **`public/og.webp`**.
...
- **Act III RSVP** (`index.html` + `style.css` + `main.js` + **`vite.config.js`**): **Root `.env`** required (`VITE_TG_BOT_TOKEN`, `VITE_TG_CHAT_ID`) — see **§ `.env` at project root** above. `.route-link` → Yandex Maps (venue `text` query). **`#btn-reveal-rsvp`** → **GSAP** reveal (`ScrollTrigger.refresh()` on complete). **Telegram**: switched from **`api.allorigins.win`** to **native Vite proxy** — `fetch('/api/telegram/bot…/sendMessage?…')`; **`server.proxy` / `preview.proxy`** in `vite.config.js` rewrites **`/api/telegram` → `https://api.telegram.org`**. **`console.log`**: env check + **`TG Response Raw`**; JSON parse; success if **`res.ok && data.ok`**; **`RSVP Fatal Error`** + *«Ошибка отправки. Проверьте консоль.»* on failure. Optional **`POST /api/rsvp`** middleware remains for an alternate server-side path.
