...
- **Mobile hero-name auto-fit:** on `(max-width: 767px) and (pointer: coarse)`, `main.js` runs `fitHeroNamesToViewport()` to auto-shrink `#hero-names` font-size until single-line title width fits viewport (with side padding), preventing horizontal overflow on small devices.
