...
- **Act III RSVP** (`index.html` + `style.css` + `main.js` + **`vite.config.js`**): **Root `.env`** required (`VITE_TG_BOT_TOKEN`, `VITE_TG_CHAT_ID`) — see **§ `.env` at project root** above. `.route-link` → Yandex Maps (venue `text` query). **`#btn-reveal-rsvp`** → **GSAP** reveal (`ScrollTrigger.refresh()` on complete). Delivery flow is now two-step for static hosting compatibility: first try **`POST /api/rsvp`** (works in Vite dev/preview or with real backend), then fallback to direct Telegram GET with **`mode:'no-cors'`** for GitHub Pages static deploys. On any hard failure, logs **`RSVP Fatal Error`** + shows *«Ошибка отправки. Проверьте консоль.»*.
