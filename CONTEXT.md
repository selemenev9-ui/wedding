...
- **Open Graph / share**: `index.html` head uses absolute HTTPS metadata for stable crawlers/cards: `og:url` = **`https://katyartemwedding.ru/`**, `og:site_name`, `og:image` + `og:image:secure_url` = **`https://katyartemwedding.ru/og.webp?v=3`**, `og:image:type` `image/webp`, dimensions **1200×630**, `og:type: website`. Added `twitter:card=summary_large_image` + `twitter:title/description/image`. Primary asset `public/og.webp` is restored to the original approved artwork; auxiliary files kept for ops: `public/og-original.webp` (backup source) and `public/og-telegram.webp` (Telegram-safe variant).
