...
| **`HeroText.js`** | В конструкторе в сцену сразу уходят **`root` / `group`** (пустой каркас, **`group.scale = 0`**). **`this.ready = _init()`** → **`waitFor('heroText')`** → **`_buildFromGltf`**. Ошибка загрузки ловится в **`_init`** (лог); **`ready`** всё равно **завершается** — прелоадер не зависает, но 3D-имена могут не появиться. |
...
- **Intro (layered)**: **`bindGlassRingScrollEffects`** + **параллакс** вешаются **до** снятия прелоадера (группы уже в сцене). Прелоадер → **`launchExperience`** → **`await heroText.ready`** → fade → **`runHeroIntro`**: **`lenis.start()`**, **`heroIntroTimeline`** (**`defaults: { ease: 'power3.out' }`**). Кольца: **`elastic.out`** по **`mesh.scale`** (в таймлайне или отложенно); **`HeroText.group.scale`** `0→1` **`expo.out`** (~1.15s) когда WebGL hero включён; **`.hero-bottom`**, мобильный **`#hero-names`**, **`.hero-tagline`** SplitType, индикатор скролла — как раньше. **`isHeroMobile` в `runHeroIntro`** — **`matchMedia('(max-width: 767px) and (pointer: coarse)')`**. **`ScrollTrigger.refresh()`** по завершении интро. **`gsap.ticker`**: Lenis + **`world.update()`** + кольца/параллакс.
...
- **HeroText** (`src/gl/world/HeroText.js`): **`this.root`** / **`this.group`** public **`THREE.Group`** — каркас в сцене в конструкторе; glTF подмешивается асинхронно (**`resources.waitFor('heroText')`**, **`this.ready`**). **`hero_text_opt.glb`** — **`_normalizeToScene(gltf.scene, 2.8)`**, **`root.position`** см. код (**`ROOT_Y`/`ROOT_Z`**), **`group.scale`** `0` → intro GSAP (**`runHeroIntro`**). **`root.visible`** — **`main.js`** **`setupHeroTextMedia`**. **Materials:** **`truffleMaterial`** — **`MeshStandardMaterial`** `#120c08`, **`roughness 1`**, **`metalness 0`**, **`envMapIntensity 0`**, **`envMap null`**, **`side: FrontSide`**, **`dithering: false`**; **`goldMaterial`** — **`MeshPhysicalMaterial`** `#e0b354`, старт **`envMapIntensity: 0`**, целевая **1.5** после **`World.tryFadeEnvReflections()`**; **`_syncGoldEnvMap`** только вешает **`envMap`**. Traverse: **`ampersand`** в **`mesh.name`** / **`material.name`** → gold; иначе truffle. **`rotation.x = π/2`**, **`castShadow: false`**, **`receiveShadow: false`**.
...
- Ensure **`public/models/hero_text_opt.glb`** exists (**Meshopt**). Если загрузка падает, **`waitFor('heroText')`** **reject** — **`HeroText._init`** логирует ошибку и **`heroText.ready`** всё равно завершается (прелоадер снимается, 3D-текст может отсутствовать). **`resources:ready`** всё равно уходит после **`allSettled`**, с **`detail.failed > 0`** при частичных ошибках (см. §1b).
