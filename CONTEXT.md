...
- **Info section (Act III DOM beat):** new `#section-info` sits between destination and final, with three editorial info cards for dress code, gifts, and children policy. Cards live entirely in the DOM layer and reveal via an independent `ScrollTrigger` in `Scroll.js`; `masterTl` WebGL choreography remains untouched.
