...
- **Lenis** defaults: `duration:2.0`, exponential easing, `wheelMultiplier:0.8`, `smoothWheel:true`, `syncTouch:true`, `touchMultiplier:1.0`, `autoRaf:false`. Coarse-pointer override in constructor: `duration:1.2`, `wheelMultiplier:1` for less inertial lag and better tap responsiveness on mobile. **Single frame source:** `gsap.ticker.add` drives **`lenis.raf(time * 1000)`** then **`world.update()`**, `glimpseGallery`, `galleryRibbon`, `glassRing`, `mouseParallax`, and scroll-velocity smoothing — **no separate `requestAnimationFrame`** loop (avoids micro-desync vs WebGL).
...
- **Mobile interaction + viewport stability**: `canvas` now uses `pointer-events:none` so touch targets are never intercepted by the fixed WebGL layer. Coarse-pointer CSS applies `100svh/100dvh` heights for hero/path/glimpse/final wrappers (`#hero-spacer`, sections, `.path-container`, `.glimpse-pin-container`, `.golden-thread`) to remove bottom white-strip artifacts during browser UI bar changes. Touch controls (`a`, `button`, `.editorial-btn`, `.route-link`, `.rsvp-radio-label`, `.rsvp-input`) now use `touch-action: manipulation` and disabled tap highlight for immediate interaction.
