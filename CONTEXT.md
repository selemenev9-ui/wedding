...
- **Event details (live copy):** invitation date/time updated to **19 September 2026, 16:00**; destination switched to restaurant **"НАЛ"** (Советская ул., 1, село Большие Салы).
...
- **Mobile readability + tactile palette v2:** destination/hero/countdown micro-copy received a stronger phone pass (`650` weight, tighter tracking, darker ink) to stay readable without enlarging the editorial titles. Dresscode swatches now behave like tactile couture controls: a glass/porcelain pill rail, soft convex color buttons with internal highlights/shadows, active gold ring state, and JS-managed `.dresscode-swatch--active` sync during palette morphs.
- **Dresscode desktop compact + quieter palette:** desktop dresscode shell was reduced in vertical weight (smaller padding/radius/shadow, narrower content, smaller title/text rhythm) so palette controls remain visible beside the dress. Swatches were moved away from an app-like pill rail into separate floating jewelry discs with subtle volume and a thin active gold ring, preserving discoverability without breaking the cinematic invitation mood.
- **Hero mobile date legibility + preload cleanup:** mobile hero date/time are intentionally larger and heavier for guest readability. `/models/dress.glb` is no longer preloaded in `index.html` because the dress model is lazy-loaded on dresscode section entry; this removes Chrome's "preloaded but not used" warning while keeping the hero path focused on rings/HDRI.
